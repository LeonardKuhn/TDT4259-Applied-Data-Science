...
forecast_date = datetime(2023, 3, 17)
...
for date in date_range:
    actual_consumption_val = data[data['time'] == date]['consumption'].values
    if len(actual_consumption_val) > 0:
        actual_consumption.append(actual_consumption_val[0])
    else:
        # Handle missing values if any
        actual_consumption.append(np.nan)
...
plt.plot(date_labels, predicted_consumption, marker='v', linestyle='-', label='Predicted')
plt.plot(date_labels, actual_consumption, marker='o', linestyle='-', label='Actual', color='orange')
